...
DEFAULT_CONFIG = {
    ...
    "cloudflare_path_messages": "/admin/mails",
    # 默认使用 tempmail.lol 免费档（免 key）；被拒收时 failover 自动轮换
    "email_provider": "tempmail_lol",
    # 邮箱故障自动切换：按 email_providers 顺序轮换（只使用已配置可用的源）
    # 免 key 源（tempmail_lol/qxfy/moemail）优先，自建/付费源靠后兜底
    "email_failover": True,
    "email_providers": [
        "tempmail_lol","qxfy",
        "cfworker","maliapi","luckmail","skymail","cloudmail","freemail","opentrashmail","laoudo","yyds",
    ],
...
def get_configured_proxy():
    # CI/无头下常用 HTTPS_PROXY/HTTP_PROXY 环境变量注入（config.proxy 优先）
...
def add_token_to_grok2api_remote_pool(raw_token, email="", log_callback=None):
    ...
    if not token:
        return False
    # CI 环境变量优先；config.json 兜底（本地 GUI 模式）
    base = os.environ.get("SUB_API_URL", "").strip().rstrip("/") or str(
        config.get("grok2api_remote_base", "") or ""
    ).strip().rstrip("/")
    admin_user = os.environ.get("SUB_ADMIN_USER", "").strip() or str(
        config.get("grok2api_admin_user", "") or ""
    ).strip()
    admin_pass = os.environ.get("SUB_ADMIN_PASS", "").strip() or str(
        config.get("grok2api_admin_pass", "") or ""
    ).strip()
    # 新版 grok2api：admin 登录 + console/import（接受 sso_token，服务端自行处理）
    if base and admin_user and admin_pass:
        try:
            login = http_post(
            ...
            access = login.json()["data"]["tokens"]["accessToken"]
            entry = json.dumps(
                {"accounts": [{"sso_token": token, "email": email}]},
                ensure_ascii=False,
            )
            # curl_cffi 不支持 requests 的 files= 参数，用 CurlMime 构造 multipart
            ...
            resp = http_post(
                f"{base}/api/admin/v1/accounts/console/import",
                multipart=mime,
            ...
            resp.raise_for_status()
            if log_callback:
                log_callback(f"[+] 已推送 grok2api console/import: {email or token[:16]}")
            # sso → OIDC token（复用 sso2cpa_core），推 Build 池；best-effort 失败不阻塞 sso
            try:
                from sso2cpa_core import convert_one
                entry = convert_one(token, email=email, proxy=get_configured_proxy())
                build_entry = {
                    "accounts": [
                        {
                            ...
                            "email": email,
                            "user_id": entry.get("sub", ""),
                            "principal_id": entry.get("sub", ""),
                        }
                    ]
                }
                mime_build = CurlMime()
                mime_build.addpart(
                    ...
                    content_type="application/json",
                    filename="auth.json",
                    data=json.dumps(build_entry, ensure_ascii=False).encode("utf-8"),
                )
                http_post(
                    f"{base}/api/admin/v1/accounts/import",
                    multipart=mime_build,
                    headers={"Authorization": f"Bearer {access}"},
                    timeout=30,
                ).raise_for_status()
                if log_callback:
                    log_callback(f"[+] 已推送 grok2api Build 池: {email or token[:16]}")
            except Exception as exc:
                if log_callback:
                    log_callback(f"[!] grok2api Build 池推送失败（不影响 sso）: {exc}")
            return True
        except Exception as exc:
            if log_callback:
                log_callback(f"[!] grok2api console/import 推送失败: {exc}")
            return False
    if log_callback:
        log_callback("[Debug] grok2api 远端未配置 base/admin_user/admin_pass，跳过")
    return False
...
def add_token_to_grok2api_pools(raw_token, email="", log_callback=None):
    ...
    # CI：SUB_API_URL 存在即视为开启远端推送（config.json 读不到环境变量）
    remote_on = bool(os.environ.get("SUB_API_URL", "").strip()) or bool(
...
def _cold_retry(method, url, request_kwargs, delay=2.0):
    """TLS 失败后冷连接重试：短退避 + 全新会话，绕开 Cloudflare 对复用握手指纹的间歇 RST。"""
    try:
        time.sleep(delay)
    except Exception:
        pass
    try:
        return _requests_fallback(method, url, **request_kwargs)
    except Exception:
        return None
...
def http_get(url, **kwargs):
    ...
    try:
        ...
        if _is_tls_or_ca_error(exc):
            resp = _cold_retry("GET", url, request_kwargs)
            if resp is not None:
                return resp
            try:
    ...
    try:
        ...
        if "certificate verify locations" in err or "curl: (77)" in err:
            ...
            try:
                ...
                err = str(exc2 or "")
        if _is_tls_or_ca_error(exc):
            resp = _cold_retry("POST", url, request_kwargs)
            if resp is not None:
                return resp
            try:
...
def cloudflare_get_messages(api_base, token, address=""):
    # 该 worker 收件列表是 /admin/mails + x-admin-auth + address 精确匹配；
    # 旧 /api/mails + Bearer 契约会被拒为 "Invalid address credential"
    headers = cloudflare_build_headers(content_type=False)
    path = get_cloudflare_path("cloudflare_path_messages", "/admin/mails")
...
def cloudflare_get_oai_code(
    ...
    if _MAILBOX_CORE_OK and wait_code_from_messages is not None:
        ...
        def list_messages():
            ...
            if resend_callback and time.time() >= next_resend_at:
                ...
                next_resend_at = time.time() + 35
            return cloudflare_get_messages(api_base, dev_token, address=email) or []
    ...
    while time.time() < deadline:
        ...
        if resend_callback and time.time() >= next_resend_at:
            ...
            next_resend_at = time.time() + 35
        try:
            messages = cloudflare_get_messages(api_base, dev_token, address=email)
        except Exception as exc:
            if log_callback:
...
# ---------------------------------------------------------------------------
# 多线程浏览器会话：每线程独立 browser/page（threading.local）。
# 单线程/GUI 路径行为不变——主线程的 TLS 槽位就是它自己的。
# 注册链路内所有函数改用 get_browser()/get_page() 取当前线程实例；
# 写实例一律走 set_browser_session()，不再 `global browser, page` 赋值。
# ---------------------------------------------------------------------------
...
def _bs():
    """当前线程的浏览器状态槽位。"""
    if not hasattr(_browser_tls, "browser"):
        _browser_tls.browser = None
        _browser_tls.page = None
    return _browser_tls
...
def set_browser_session(br=None, pg=None):
    _bs().browser = br
    _bs().page = pg
...
def refresh_active_page():
    ...
    try:
        ...
        if tabs:
            ...
            pg = get_page()  # 保留现有页，避免 new_tab 造 about:blank 覆盖真实页
        set_browser_session(br, pg)
        page = pg
        return pg
    except Exception:
...
def open_signup_page(log_callback=None, cancel_callback=None):
    ...
    if browser_started_with_proxy and page_has_proxy_error(get_page()):
        if allow_fallback:
            if log_callback:
...
def fill_email_and_submit(timeout=45, log_callback=None, cancel_callback=None):
    ...
    # snapshot BEFORE xAI submit — OTP often arrives in the gap before wait starts
...
return false;
    ...
    # prefer pre-submit snapshot from fill_email_and_submit; only re-snap if missing
    if before_ids is None:
        before_ids = snapshot_inbox_ids(dev_token, email=email, log_callback=log_callback)
    if otp_sent_at is None:
        # slight skew allowance — mail may be stamped just before local submit clock
...
_SOLVER_URL = "http://127.0.0.1:5072"
_SITEKEY_CACHE = ""


def _cache_sitekey(log_callback=None) -> str:
    """Grab the Turnstile sitekey. It's embedded in the raw HTML (RSC JSON), not
    in DOM attributes — React lazily mounts the widget, so [data-sitekey] and the
    iframe often don't exist yet. Scan page.html instead of DOM.
    """
    import re as _re

    global _SITEKEY_CACHE
    if _SITEKEY_CACHE:
        return _SITEKEY_CACHE
    if get_page() is None:
        return _SITEKEY_CACHE
    try:
        html = get_page().html or ""
        m = _re.search(r'["\']sitekey["\']\s*:\s*["\'](0x[0-9a-fA-F]{28,})["\']', html)
        if m:
            _SITEKEY_CACHE = m.group(1)
    except Exception:
        pass
    if not _SITEKEY_CACHE:
        # accounts.x.ai/sign-up 的 Turnstile sitekey 为静态常量（抓包确认）。
        # RSC 懒加载时序下 HTML 可能未含 sitekey 文本，回退避免 solver 被跳过。
        _SITEKEY_CACHE = "0x4AAAAAAAhr9JGVDZbrZOo0"
    if log_callback:
        log_callback(f"[Debug] 本地打码: sitekey={_SITEKEY_CACHE[:12]}...")
    return _SITEKEY_CACHE


def _solver_solve(log_callback=None, cancel_callback=None, timeout: float = 90.0) -> str:
    """Ask local Turnstile solver (:5072, YesCaptcha/CapSolver protocol) for a token.

    Returns token string, or "" if solver is down / can't solve.
    """
    if get_page() is None:
        return ""
    page_url = ""
    sitekey = ""
    try:
        page_url = str(get_page().url or "")
    except Exception:
        page_url = ""
    if not page_url:
        return ""
    sitekey = _cache_sitekey(log_callback=log_callback)
    if not sitekey:
        if log_callback:
            log_callback("[Debug] 本地打码: 页面未找到 sitekey，跳过")
        return ""
    try:
        import requests as _r

        s = _r.Session()
        # 同一 taskId 重查只会重现同样 error（ERROR_CAPTCHA_UNSOLVABLE 等）。
        # 失败后重建新任务：接力池里不同的 worker 可能可解。
        for attempt in range(1, 4):
            if attempt > 1:
                if log_callback:
                    log_callback(f"[*] 本地打码: 第 {attempt}/3 次重建任务（上次无解）")
                sleep_with_cancel(2, cancel_callback)
            create = s.post(
                f"{_SOLVER_URL}/createTask",
                json={
                    "task": {
            ...
            task_id = str((create or {}).get("taskId") or "")
            if not task_id:
                if log_callback:
                    log_callback(f"[!] 本地打码 createTask 失败: {create}")
                continue
            if log_callback:
                log_callback(f"[*] 本地打码任务已创建[{attempt}]: {task_id[:8]}... sitekey={sitekey[:12]}")
            deadline = time.time() + timeout
            while time.time() < deadline:
                raise_if_cancelled(cancel_callback)
                res = s.post(
                    f"{_SOLVER_URL}/getTaskResult",
                    json={"taskId": task_id},
                    timeout=15,
                ).json()
                status = str((res or {}).get("status") or "")
                if status == "ready":
                    tok = str(((res or {}).get("solution") or {}).get("token") or "")
                    if len(tok) >= 80:
                        return tok
                    break  # 短 token 视为无效，重建任务
                if str((res or {}).get("errorId") or "0") != "0":
                    if log_callback:
                        log_callback(f"[!] 本地打码任务失败: {res.get('errorCode')}")
                    break
                sleep_with_cancel(3, cancel_callback)
        if log_callback:
            log_callback("[!] 本地打码 3/3 次均无解")
        return ""
    except Exception as exc:
        if log_callback:
            log_callback(f"[Debug] 本地打码不可用: {exc}")
        return ""


def _solver_backfill_token(token, log_callback=None) -> bool:
    """Put solver token into the page's Turnstile response input."""
    if not token or get_page() is None:
        return False
    try:
        get_page().run_js(
            """(tok) => {
  const setInput = (el) => { el.value = tok; el.dispatchEvent(new Event('input', {bubbles:true})); };
  ...
  if (window.turnstile && typeof window.turnstile.getResponse === 'function') {
    try { window.turnstile.reset(); } catch(e) {}
        ...
        if log_callback:
            log_callback(f"[*] 本地打码 token 已回填 (长度={len(token)})")
        return True
    except Exception as exc:
        if log_callback:
            log_callback(f"[Debug] 回填失败: {exc}")
        return False
...
def getTurnstileToken(log_callback=None, cancel_callback=None, allow_reset=False):
    ...
    _cache_sitekey()                       # 原生点击前缓存 sitekey
    ...
    for attempt in range(0, 24):
        ...
        try:
            token = ""
            if hasattr(pg, "get_turnstile_token"):
                try:
                    token = str(pg.get_turnstile_token() or "").strip()
                except Exception:
                    token = ""
            if not token:
                token = get_page().run_js(
                    """
try {
  ...
  return '';
            ...
            if hasattr(pg, "click_turnstile"):
                try:
                    detail = pg.click_turnstile() or {}
                    ...
                    if not token2 and hasattr(pg, "get_turnstile_token"):
                        token2 = str(pg.get_turnstile_token() or "").strip()
                    if len(token2) >= 80:
                        if log_callback:
            ...
            challenge_input = get_page().ele("@name=cf-turnstile-response")
            if challenge_input:
...
if (nodes.length && typeof nodes[0].click === 'function') nodes[0].click();
    ...
    # 原生途径拿不到 token → 交给本地打码 (:5072)，失败再抛
    tok = _solver_solve(log_callback=log_callback, cancel_callback=cancel_callback)
    if _solver_backfill_token(tok, log_callback=log_callback):
        return tok
            ...
            if isinstance(filled, str) and filled.startswith("wait-cloudflare"):
                ...
                if token_len == "0":
                    ...
                    if wait_cf_since is None:
                        wait_cf_since = now0
                        _cache_sitekey(log_callback=log_callback)  # widget 还在,立刻缓存 sitekey
                    # Managed Turnstile often auto-solves if we wait; avoid spam-clicking early.
                    waited = now0 - wait_cf_since
                    if waited >= 6 and hasattr(pg, "click_turnstile"):
                        try:
                            detail = pg.click_turnstile() or {}
                            if log_callback:
                                log_callback(
...
return 'submitted';
        ...
        if isinstance(submit_state, str) and submit_state.startswith("wait-cloudflare"):
            ...
            if (
                hasattr(get_page(), "click_turnstile")
                and now - wait_cf_since >= 8
                and now - last_cf_retry_at >= 6
            ):
                try:
                    get_page().click_turnstile()
                except Exception:
                    pass
...
def _iter_cookie_sources():
    ...
    try:
        _br = get_browser()
        if _br is not None and hasattr(_br, "get_tabs"):
            for tab in _br.get_tabs() or []:
                try:
                    url = tab.url or ""
                except Exception:
                    url = ""
    ...
    for url, tab in tabs:
        if "grok.com" not in (url or ""):
            ordered.append(tab)
    if get_page() is not None and get_page() not in ordered:
        ordered.append(get_page())
...
def wait_for_grok_com_landing(timeout=90, log_callback=None, cancel_callback=None):
    ...
    while time.time() < deadline:
        ...
        try:
            ...
            try:
                br = get_browser()
                if br is not None and hasattr(br, "get_tabs"):
                    for tab in br.get_tabs() or []:
                        try:
                            url = tab.url or ""
                        except Exception:
                            url = ""
                        if "grok.com" in url:
                            set_browser_session(br, tab)
                            break
            except Exception:
                pass
...
def wait_for_sso_cookie(
    ...
    while time.time() < deadline:
        ...
        try:
            refresh_active_page()
            now = time.time()
            if log_callback and now - last_page_diag >= 15:
                last_page_diag = now
                try:
                    _pg_url = (get_page().url or "") if get_page() is not None else ""
                except Exception:
                    _pg_url = ""
                _tab_urls = []
                try:
                    _br = get_browser()
                    if _br is not None and hasattr(_br, "get_tabs"):
                        for _t in (_br.get_tabs() or []):
                            try:
                                _tab_urls.append(_t.url or "")
                            except Exception:
                                _tab_urls.append("?")
                except Exception:
                    pass
                log_callback(
                    f"[Debug] 页面诊断: page={_pg_url[:120]} | tabs={_tab_urls[:5]}"
                )
            if page is None:
                sleep_with_cancel(1, cancel_callback)
                continue
...
return String(cfInput.value || '').trim().length;
            ...
            try:
                cur = (get_page().url or "") if get_page() is not None else ""
            except Exception:
                cur = ""
            left_signup = bool(
                cur
...
class GrokRegisterGUI:
    ...
    def setup_ui(self):
        ...
        add_label(9, 0, "grok2api 远端 admin 账号:")
        self.grok2api_admin_user_var = tk.StringVar(value=str(config.get("grok2api_admin_user", "")))
        self.grok2api_admin_user_entry = tk_entry(config_frame, textvariable=self.grok2api_admin_user_var, width=72)
        add_field(self.grok2api_admin_user_entry, 9, 1, columnspan=3)

        add_label(10, 0, "grok2api 远端 admin 密码:")
        self.grok2api_admin_pass_var = tk.StringVar(value=str(config.get("grok2api_admin_pass", "")))
        self.grok2api_admin_pass_entry = tk_entry(config_frame, textvariable=self.grok2api_admin_pass_var, width=72)
        add_field(self.grok2api_admin_pass_entry, 10, 1, columnspan=3)
    ...
    def start_registration(self):
        ...
        config["grok2api_admin_pass"] = self.grok2api_admin_pass_var.get().strip()
                ...
                try:
                    ...
                    for mail_try in range(1, max_mail_retry + 1):
                        try:
                            ...
                            self.log("[*] 2. 创建邮箱并提交")
                            email, dev_token, before_ids = fill_email_and_submit(
                                log_callback=self.log, cancel_callback=cancel_cb
                            )
                            self.log(f"[*] 邮箱: {email}")
...
def _rotate_tab(log):
    """换新 tab 隔离账号（cookie 不串）；失败才整浏览器重启。"""
    br = get_browser()
    if br is not None:
        try:
            pg = br.new_tab(SIGNUP_URL)
            ...
            log(f"[Debug] 换 tab 失败: {exc}")
    restart_browser(log_callback=log)


def _register_one(index, accounts_file, controller, log, stats, lock):
    """在调用线程的浏览器槽位内注册一个账号。返回是否成功。"""
    # ponytail: fresh browser per account; shared session leaks logged-in /account page
    # across accounts. Per-account restart if browser startup cost ever matters.
    restart_browser(log_callback=log)
    ...
    log(f"--- 开始第 {index} 个账号 · 超时 {round_timeout}s ---")
    _rotate_tab(log)
    email = ""
    dev_token = ""
    code = ""
    ...
    try:
        for mail_try in range(1, max_mail_retry + 1):
            try:
                ...
                open_signup_page(
                    log_callback=log, cancel_callback=cancel_cb
                )
                log("[*] 2. 创建邮箱并提交")
                email, dev_token, before_ids = fill_email_and_submit(
                    log_callback=log, cancel_callback=cancel_cb
                ...
                if is_mail_related_error(mail_exc) and mail_try < max_mail_retry:
                    ...
                    if config.get("email_failover", True):
                        rotate_email_provider(
                            log_callback=log, reason=msg[:120]
                        )
                    _rotate_tab(log)
                    sleep_with_cancel(1, cancel_cb)
                    continue
        ...
        log("[*] 5. 等待落到 grok.com 并提取 sso cookie（优先 grok.com 域）")
        sso = wait_for_sso_cookie(
            log_callback=log, cancel_callback=cancel_cb
        )
        if config.get("enable_nsfw", True):
        ...
        with lock:
            ...
            n = stats["success"]
        log(f"[+] 注册成功: {email}")
        if n > 0 and n % MEMORY_CLEANUP_INTERVAL == 0:
            cleanup_runtime_memory(
                log_callback=log,
                reason=f"已成功 {n} 个账号，执行定期清理",
            )
        return True
    except AccountRetryNeeded as exc:
        log(f"[-] 当前账号流程卡住: {exc}")
...
def run_registration_cli(count, threads=1):
    ...
    cli_log(f"[*] 单账号硬超时: {get_round_timeout_sec()}s")
    ...
    try:
        if threads <= 1:
            for i in range(1, count + 1):
                if controller.should_stop():
                    break
                _register_one(i, accounts_output_file, controller, cli_log, stats, lock)
        else:
            def _run(i):
                if controller.should_stop():
                    return
            ...
            with ThreadPoolExecutor(max_workers=threads) as ex:
                futures = [
                    ex.submit(_run, i) for i in range(1, count + 1)
                ]
                for f in as_completed(futures):
                    try:
                        f.result()
                    except Exception as exc:
                        cli_log(f"[!] 任务线程异常: {exc}")
        ...
        cli_log(f"[*] 任务结束。成功 {stats['success']} | 失败 {stats['fail']}")


def main_cli():
    load_config()
    p = argparse.ArgumentParser(prog="grok_register_ttk", add_help=False)
    p.add_argument("--count", type=int)
    p.add_argument("--threads", type=int)
    p.add_argument("--email")
    # 保留兼容：main() 会剥掉 `cli` 作为 argv[1]，除非显式传参
    ...
    if known.count is not None and known.count > 0:
        count = int(known.count)
    elif env_count:
        try:
        ...
        count = int(config.get("register_count", 1) or 1)
    threads = int(known.threads) if known.threads else int(config.get("threads") or 1)
    if known.email:
        config["email_provider"] = str(known.email).strip()
    cli_log("[*] CLI 已加载配置")
    cli_log(f"[*] 当前邮箱服务商: {config.get('email_provider', '')} | 数量: {count} | 并发: {threads}")
    # CI/action 下 stdin 非 TTY：跳过 "start" 确认，直接开跑（input() 在 EOF 会抛 EOFError）
    # 显式传 --count 也视为非交互：调用方已表明直接执行意图
    if not sys.stdin.isatty() or known.count is not None:
        cli_log("[*] 非交互启动（stdin 非 TTY），直接开始")
        run_registration_cli(count, threads)
        return
...
def main():
    if len(sys.argv) > 1 and (sys.argv[1].strip().lower() in ("start", "cli", "--cli") or sys.argv[1].startswith("-")):
        main_cli()
        return
    root = tk.Tk()
