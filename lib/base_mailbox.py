...
class BaseMailbox(ABC):
    ...
    def _safe_extract(self, text: str, pattern: str = None) -> Optional[str]:
        ...
        # xAI/Grok: "confirmation code: 1J1-HA3"；再退到 6 位数字
        patterns.extend(
            [
                r"(?is)(?:confirmation|verification|security|login|one[-\s]*time)\s+code[^A-Z0-9]{0,20}([A-Z0-9]{3}-[A-Z0-9]{3})",
...
def create_mailbox(
    ...
    if provider == "tempmail_lol":
        ...
        return QxfyMailbox(
            api_url=extra.get("qxfy_api_url") or "https://qxfy.store",
            domain=extra.get("qxfy_domain") or "",
            proxy=proxy,
        )
...
class QxfyMailbox(BaseMailbox):
    """qxfy.store 公益 KV 临时邮箱：本地拼地址 + 轮询收信。"""

    def __init__(
        self,
        api_url: str = "https://qxfy.store",
        domain: str = "",
        proxy: str = None,
    ):
        self.api = str(api_url or "https://qxfy.store").strip().rstrip("/") or "https://qxfy.store"
        self.domain = str(domain or "").strip().lstrip("@").lower()
        self.proxy = build_requests_proxy_config(proxy)

    @staticmethod
    def filter_domains(raw) -> list[str]:
        """Keep usable catch-all domains; drop gmail / empty / non-domain names."""
        out: list[str] = []
        seen: set[str] = set()
    ...
    def message_id(message: dict) -> str:
        ...
        date = str(
            message.get("date") or message.get("createdAt") or message.get("time") or ""
        ).strip()
        sender = str(
            message.get("from")
        ...
        # empty/placeholder rows must not get a stable phantom hash
        if not (subject or date or sender):
            return ""
        raw = f"{subject}|{date}|{sender}"
    ...
    def _request_json(self, method: str, path: str, timeout: int = 15) -> Any:
        ...
        try:
            ...
            preview = (response.text or "")[:200]
            raise RuntimeError(
                f"Qxfy API {path} 非 JSON: HTTP {response.status_code} {preview}"
            ) from exc
        if response.status_code >= 400:
            raise RuntimeError(
                f"Qxfy API {path} 失败: HTTP {response.status_code} {str(data)[:200]}"
            )
        return data

    def _list_domains(self) -> list[str]:
        data = self._request_json("GET", "/api/domains", timeout=15)
        domains = []
        if isinstance(data, dict):
            domains = data.get("domains") or []
        elif isinstance(data, list):
            domains = data
        return self.filter_domains(domains)

    def _list_messages(self, email: str) -> list[dict]:
        from urllib.parse import quote

        data = self._request_json("GET", f"/api/emails/{quote(email, safe='@.')}", timeout=15)
        if isinstance(data, dict):
    ...
    def _delete_message(self, email: str, message_id: str) -> None:
        from urllib.parse import quote
        ...
        try:
            self._request_json(
                "DELETE",
                f"/api/emails/{quote(email, safe='@.')}/{quote(mid, safe='')}",
                timeout=10,
            )
        except Exception:
            pass
    ...
    def _build_search_text(self, message: dict) -> str:
        parts = []
        # verificationCode first — qxfy/violetteam pre-extracts OTP
        for key in (
            "verificationCode",
            "verification_code",
            "code",
    ...
    def get_email(self) -> MailboxAccount:
        ...
        if not domain:
            ...
            if not domains:
                raise RuntimeError("Qxfy 无可用域名（/api/domains 过滤后为空）")
            domain = random.choice(domains)
        email = f"{self._generate_local_part()}@{domain}"
        self._log(f"[Qxfy] 生成邮箱: {email}")
        return MailboxAccount(
            email=email,
            account_id=email,
            extra={"provider": "qxfy", "domain": domain},
        )
    ...
    def get_current_ids(self, account: MailboxAccount) -> set:
        try:
            return {
                self.message_id(msg)
                for msg in self._list_messages(account.email)
                if self.message_id(msg)
            }
        except Exception:
            return set()
        ...
        def poll_once() -> Optional[str]:
            try:
                for message in self._list_messages(account.email):
                    ...
                    msg_ts = None
                    try:
                        from mailbox_core import message_timestamp as _mts

                        msg_ts = _mts(message)
                    except Exception:
                        msg_ts = None
                    if otp_sent_at and msg_ts and float(msg_ts) < float(otp_sent_at):
                        continue

                    # direct field (violetteam API)
                    for key in (
                        "verificationCode",
                        "verification_code",
                        "code",
                        ...
                        raw_code = str(message.get(key) or "").strip()
                        if raw_code and raw_code not in exclude_codes:
                            code = self._safe_extract(raw_code, code_pattern) or raw_code
                            if code and code not in exclude_codes:
                                self._log(f"[Qxfy] 收到验证码: {code}")
                                self._delete_message(account.email, mid)
                                return code
...
class CFWorkerMailbox(BaseMailbox):
    ...
    def _request_json(
        ...
        *,
        ...
        url = f"{self.api}{path}"
        headers = self._headers()
        # workers.dev 经本地 MITM 代理时常 Hostname mismatch；先直连，再代理，最后 verify=False
        attempts = []
        if self.proxy:
            attempts.append({"proxies": None, "verify": True, "tag": "direct"})
            attempts.append({"proxies": self.proxy, "verify": True, "tag": "proxy"})
            attempts.append({"proxies": None, "verify": False, "tag": "direct-insecure"})
            attempts.append(
                {"proxies": self.proxy, "verify": False, "tag": "proxy-insecure"}
            )
        else:
            attempts.append({"proxies": None, "verify": True, "tag": "direct"})
            attempts.append({"proxies": None, "verify": False, "tag": "direct-insecure"})
        ...
        for attempt in attempts:
            try:
                ...
                continue
        if response is None:
            raise RuntimeError(f"CFWorker API {path} 网络/SSL 失败: {last_err}") from last_err
            ...
            try:
                ...
                for mail in sorted(mails, key=lambda x: x.get("id", 0), reverse=True):
                    ...
                    if not subject and raw:
                        m_subj = re.search(r"(?im)^Subject:\s*(.+)$", raw)
                        if m_subj:
                            subject = m_subj.group(1).strip()
                    # subject 常含 xAI 码（如 1J1-HA3），必须保留；raw 解码会丢掉 headers
                ...
                self._log(f"[CFWorker] \u62c9\u53d6\u90ae\u4ef6\u5931\u8d25: {exc}")
            return None
