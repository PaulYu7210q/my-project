...
MAIL_PROVIDER_CHOICES = [
    ("tempmail_lol", "Tempmail.lol (免费档免 key)"),
    ("qxfy", "Qxfy / 公益 KV 邮箱"),
...
def normalize_provider(name: str) -> str:
    ...
    aliases = {
        ...
        "cf_worker": "cfworker",
        "cloudflare": "cfworker",
        # tempmail.lol 免费档免 key 可用；failover 兜底拒收风险
...
def extra_from_config(config: dict) -> dict:
    ...
    return {
        ...
        "qxfy_api_url": str(c.get("qxfy_api_url") or "https://qxfy.store").strip(),
        "qxfy_domain": str(c.get("qxfy_domain") or "").strip(),
    }
...
def provider_ready(config: dict, provider: str) -> bool:
    ...
    if p == "moemail":
        return True
    if p == "tempmail_lol":
        return True  # 免费档免 key 即就绪
    if p == "qxfy":
        return True  # default public base URL; no key required
    if p == "maliapi":
        return bool(
